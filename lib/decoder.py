        ...
        try:
            ...
            for c in convo:
                ...
                if header in extraction:
                    ...
                    extraction[header] = open(os.path.join(self.result_dir, str(
                        batch_id), str(iter_id), header + ".ctm"), "w")
                    convo_repo[header] = []
                    ...
                    for idx, _ in enumerate(aligned_sentences):
                        if idx < (len(aligned_sentences) - 1):
                            aligned_sentences[idx].length = aligned_sentences[idx +
                                                                          1].words[0].timestamp
                    aligned_sentences[(len(aligned_sentences) - 1)
                                  ].length = duration
                transcript_out = {"duration": duration, "length": len(
                    alignment), "sentences": aligned_sentences, "complete": "1"}
                out_json = open(os.path.join("/root/audio/batch" +
                                         str(batch_id), key + ".json"), "w")
